...
import { metricsList } from './config/metrics'
...
export default function MetricsCharts() {
  ...
  const [currentCountry, setCurrentCountry] = useState<string>('global')
  ...
  useEffect(() => {
    ...
    fetch(`/api/metrics/countries?projectId=${currentProject.id}`)
      .then((res) => res.json())
      .then((arr) => {
        if (Array.isArray(arr) && arr.length > 0) {
          setCountries(arr)
          if (!arr.includes(currentCountry)) setCurrentCountry(arr[0])
        } else {
          setCountries(['global'])
          setCurrentCountry('global')
        }
      })
  }, [currentCountry, currentProject])

  // 动态拉取 path 列表
  useEffect(() => {
    if (!currentProject || !currentCountry) return
    fetch(
      `/api/metrics/paths?projectId=${currentProject.id}&country=${currentCountry}`
    )
      .then((res) => res.json())
      .then((arr) => {
        if (Array.isArray(arr) && arr.length > 0) {
  ...
  }, [currentProject, currentCountry])

  // 切换项目或国家时拉取数据
  useEffect(() => {
    if (!currentProject || !currentCountry || !currentPath) return
    setLoading(true)
    Promise.all(
      metricsList.map((metric) =>
        fetch(
          `/api/metrics?projectId=${currentProject.id}&type=${
            metric.type
          }&country=${currentCountry}&path=${encodeURIComponent(currentPath)}`
        ).then((res) => res.json())
      )
    ).then((results) => {
  ...
  }, [currentProject, currentCountry, currentPath])
    ...
    <div className="w-full min-h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-gray-950 relative overflow-hidden">
      ...
      <div className="relative w-full max-w-[1800px] mx-auto p-4 md:p-8 space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 backdrop-blur-sm py-6">
          ...
          {/* 项目选择器 + 国家选择器 + 页面选择器 */}
          <div className="flex justify-center md:justify-end z-50 gap-4">
            ...
            <SimpleSelect
              widthClass="w-60"
              ariaLabel="选择页面"
              options={paths.map((p) => ({ label: p, value: p }))}
              value={currentPath}
              onChange={(v) => setCurrentPath(v)}
              disabled={paths.length === 0}
              placeholder={paths.length === 0 ? '暂无页面' : '请选择页面'}
            />
          </div>
        </div>
